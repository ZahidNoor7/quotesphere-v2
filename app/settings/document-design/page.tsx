...
const INPUT_STYLE: React.CSSProperties = {
  ...
  borderRadius: 6, color: "#e2e8f0", fontSize: 11, padding: "5px 8px", outline: "none",
  boxSizing: "border-box", fontFamily: "inherit",
};

function TermsEditor({ text, format, onChange }: {
  text: string;
  ...
  if (format === "paragraph") {
    return (
      <textarea
        ...
        style={{ ...INPUT_STYLE, resize: "vertical" as const, lineHeight: 1.6, padding: "7px 10px" }}
      />
    );
  }
  ...
  const DragHandle = () => (
    <svg width="8" height="12" viewBox="0 0 8 12" style={{ cursor: "grab", flexShrink: 0 }} fill="rgba(255,255,255,0.2)">
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      {lines.map((line, idx) => (
        <div
          ...
          style={{
            ...
            background: dragOver === idx ? "rgba(99,102,241,0.1)" : "transparent",
            border: dragOver === idx ? "1px dashed rgba(99,102,241,0.45)" : "1px solid transparent",
            borderRadius: 6, padding: "2px 4px 2px 2px", transition: "background 0.1s",
          }}
        >
          <DragHandle />
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", flexShrink: 0, minWidth: 16, textAlign: "right", userSelect: "none" }}>
          ...
          <input
            ...
            onKeyDown={e => {
              ...
              if (e.key === "Backspace" && line === "" && lines.length > 1) { e.preventDefault(); removeLine(idx); }
            }}
          ...
          {lines.length > 1 && (
            <button
              ...
              style={{
                width: 18, height: 18, borderRadius: 4, background: "rgba(248,113,113,0.12)",
                border: "0.5px solid rgba(248,113,113,0.3)", color: "#f87171", cursor: "pointer",
      ...
      <button
        onClick={addLine}
        style={{
          width: "100%", padding: "5px 0", borderRadius: 6, fontSize: 10.5, cursor: "pointer",
          border: "0.5px dashed rgba(99,102,241,0.4)", background: "rgba(99,102,241,0.06)", color: "#818cf8",
        }}
      >+ Add item</button>
    </div>
  );
...
export default function DocumentDesignPage() {
  ...
  useEffect(() => {
    ...
    const key = activeDocType === "invoice" ? "invoiceDesignId"
    ...
    try {
      ...
      if (hadOverrides) {
        if (isUserDesign) {
          ...
          const putRes = await fetch(`/api/settings/document-designs/${selectedDesignId}`, {
            method: "PUT",
      ...
      const res = await fetch(`/api/settings/document-designs/${designIdToUse}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      toast.success(hadOverrides && hadBuiltIn
  ...
  const effectiveSampleData = activeDocType === "receipt"
    ? SAMPLE_RECEIPT_DATA
    : {
      ...SAMPLE_DATA,
      items: effectiveMockItems,
      subTotal: effectiveMockItems.reduce((s, i) => s + i.quantity * i.price, 0),
    };
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const update = () => setPreviewWidth(Math.max(1, Math.floor(el.getBoundingClientRect().width)));
    update();
  ...
  function Toggle({ configKey, label }: { configKey: keyof DocumentDesignConfig; label: string }) {
    const val = !!effectiveConfig[configKey];
    return (
      <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
        <div
          onClick={() => updateConfig({ [configKey]: !val } as Partial<DocumentDesignConfig>)}
          style={{ width: 30, height: 16, borderRadius: 100, background: val ? "#6366f1" : "rgba(255,255,255,0.1)", position: "relative", cursor: "pointer", transition: "background 0.2s", flexShrink: 0 }}
        >
          <div style={{ width: 12, height: 12, borderRadius: "50%", background: "#fff", position: "absolute", top: 2, left: val ? 16 : 2, transition: "left 0.2s" }} />
        </div>
  ...
  function SegmentButtons<T extends string>({ configKey, options, label }: { configKey: keyof DocumentDesignConfig; options: readonly { value: T; label: string }[]; label: string }) {
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" as const }}>
          {options.map(o => (
            <button key={o.value} onClick={() => updateConfig({ [configKey]: o.value } as Partial<DocumentDesignConfig>)}
              style={{
                padding: "4px 10px", borderRadius: 6, fontSize: 10, cursor: "pointer",
                border: `1px solid ${current === o.value ? "#818cf8" : GLASS_BORDER}`,
                background: current === o.value ? "rgba(99,102,241,0.18)" : GLASS,
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12 }}>
          <div style={{ width: 28, height: 28, borderRadius: "50%", border: "2.5px solid rgba(99,102,241,0.25)", borderTopColor: "#6366f1", animation: "spin 0.7s linear infinite" }} />
          <span style={{ fontSize: 12, color: T3 }}>Loading design settings…</span>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
      ...
      <div ref={bodyRef} style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ width: `${splitPct}%`, flexShrink: 0, borderRight: `0.5px solid ${GLASS_BORDER}`, overflowY: "auto", display: "flex", flexDirection: "column" }}>
          {/* Design grid */}
                  ...
                  {effectiveConfig.headerEnabled !== false && <>
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                        {["#ffffff", "#f8fafc", "#111827", "#1e293b", "#c8a96e", "#8b4513"].map(c => (
                          <button key={c} onClick={() => updateConfig({ headerTextColor: c })}
                            style={{ width: 22, height: 22, borderRadius: 5, background: c, cursor: "pointer", border: effectiveConfig.headerTextColor === c ? "2px solid #818cf8" : "1px solid rgba(255,255,255,0.2)" }}
                            title={c}
                          />
                        ))}
                        <input type="color" value={effectiveConfig.headerTextColor || "#ffffff"}
                          onChange={e => updateConfig({ headerTextColor: e.target.value })}
                          style={{ width: 22, height: 22, borderRadius: 5, border: "none", cursor: "pointer", padding: 0 }}
                          title="Custom text color"
                        />
                        {effectiveConfig.headerTextColor && (
                          <button onClick={() => updateConfig({ headerTextColor: "" })}
                            style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, border: `0.5px solid ${GLASS_BORDER}`, background: GLASS, color: T3, cursor: "pointer" }}
                          >Reset</button>
                        )}
                      </div>
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      <Toggle configKey="showLogo" label="Show company logo" />
                      {effectiveConfig.showLogo && (
                        <div>
                          <label style={lbl}>Logo URL (or upload via Company Settings)</label>
                          <input
                      ...
                      <Toggle configKey="showAddress" label="Show company address" />
                      <Toggle configKey="showPhone" label="Show company phone" />
                    </div>
                  </>}
                </div>
              )}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  {effectiveConfig.footerEnabled !== false && <>
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                        {["#ffffff", "#f8fafc", "#f1f5f9", "#0f172a", "#1e293b", "#faf7f0"].map(c => (
                          <button key={c} onClick={() => updateConfig({ footerBg: c })}
                            style={{ width: 22, height: 22, borderRadius: 5, background: c, cursor: "pointer", border: effectiveConfig.footerBg === c ? "2px solid #818cf8" : "1px solid rgba(255,255,255,0.2)" }}
                            title={c}
                          />
                        ))}
                        <input type="color" value={(effectiveConfig as any).footerBg || "#ffffff"}
                          onChange={e => updateConfig({ footerBg: e.target.value })}
                          style={{ width: 22, height: 22, borderRadius: 5, border: "none", cursor: "pointer", padding: 0 }}
                          title="Custom bg"
                        />
                        {effectiveConfig.footerBg && (
                          <button onClick={() => updateConfig({ footerBg: "" })}
                            style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, border: `0.5px solid ${GLASS_BORDER}`, background: GLASS, color: T3, cursor: "pointer" }}
                          >None</button>
                        )}
                      </div>
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                        {["#888888", "#9ca3af", "#475569", "#111827", "#ffffff", "#7c5533"].map(c => (
                          <button key={c} onClick={() => updateConfig({ footerTextColor: c })}
                            style={{ width: 22, height: 22, borderRadius: 5, background: c, cursor: "pointer", border: effectiveConfig.footerTextColor === c ? "2px solid #818cf8" : "1px solid rgba(255,255,255,0.2)" }}
                            title={c}
                          />
                        ))}
                        <input type="color" value={(effectiveConfig as any).footerTextColor || "#888888"}
                          onChange={e => updateConfig({ footerTextColor: e.target.value })}
                          style={{ width: 22, height: 22, borderRadius: 5, border: "none", cursor: "pointer", padding: 0 }}
                          title="Custom text color"
                        />
                        {effectiveConfig.footerTextColor && (
                          <button onClick={() => updateConfig({ footerTextColor: "" })}
                            style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, border: `0.5px solid ${GLASS_BORDER}`, background: GLASS, color: T3, cursor: "pointer" }}
                          >Reset</button>
                        )}
                      </div>
                    </div>
                    <Toggle configKey="showPageNumber" label="Show page number" />
                  </>}
                </div>
              )}
              ...
              {activeSettingsTab === "Content" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    <Toggle configKey="showDiscount" label="Show discount line" />
                  </div>
                  <div style={{ borderTop: `0.5px solid ${GLASS_BORDER}`, paddingTop: 10 }}>
                    <div style={secHd}>Terms & Conditions</div>
                    <Toggle configKey="showTerms" label="Enable terms & conditions" />
                    {effectiveConfig.showTerms && <>
                      ...
                      <div style={{ marginTop: 8 }}>
                        ...
                        <TermsEditor
                          ...
                          format={(effectiveConfig.termsFormat ?? "paragraph") as "paragraph" | "bullet" | "numbered"}
                  ...
                  <div style={{ borderTop: `0.5px solid ${GLASS_BORDER}`, paddingTop: 10 }}>
                    <label style={lbl}>Content gap between sections: {effectiveConfig.contentGap ?? 12}px</label>
                    <input
                      type="range" min={4} max={40} step={2}
                      value={effectiveConfig.contentGap ?? 12}
                      onChange={e => updateConfig({ contentGap: Number(e.target.value) })}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: T3, marginTop: 2 }}>
                      <span>Tight (4px)</span><span>Spacious (40px)</span>
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div style={{ borderTop: `0.5px solid ${GLASS_BORDER}`, paddingTop: 10 }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", gap: 4 }}>
                        {PAGE_SIZES.map(s => (
                          <button key={s} onClick={() => updateConfig({ pageSize: s })}
                            style={{
                              flex: 1, padding: "5px 0", borderRadius: 6, fontSize: 10.5, cursor: "pointer",
                              border: `1px solid ${effectiveConfig.pageSize === s ? "#818cf8" : GLASS_BORDER}`,
                              background: effectiveConfig.pageSize === s ? "rgba(99,102,241,0.18)" : GLASS,
                              color: effectiveConfig.pageSize === s ? "#818cf8" : T2,
                            }}
                          >{s}</button>
                        ))}
                      </div>
                    ...
                    <div style={{ marginTop: 8 }}>
                      ...
                      <div style={{ display: "flex", gap: 4 }}>
                        {ORIENTATIONS.map(o => (
                          <button key={o} onClick={() => updateConfig({ pageOrientation: o })}
                            style={{
                              flex: 1, padding: "5px 0", borderRadius: 6, fontSize: 10.5, cursor: "pointer",
                              border: `1px solid ${effectiveConfig.pageOrientation === o ? "#818cf8" : GLASS_BORDER}`,
                              background: effectiveConfig.pageOrientation === o ? "rgba(99,102,241,0.18)" : GLASS,
                              color: effectiveConfig.pageOrientation === o ? "#818cf8" : T2,
                              textTransform: "capitalize",
                            }}
                          >{o}</button>
                        ))}
                    ...
                    <div style={{ marginTop: 6, padding: "6px 8px", borderRadius: 5, background: "rgba(99,102,241,0.06)", border: `0.5px solid rgba(99,102,241,0.15)` }}>
                      <span style={{ fontSize: 9.5, color: T3 }}>
                        Page size & orientation apply to PDF export. Preview always shows A4 portrait scale.
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div style={{ padding: "10px 14px 14px", borderTop: `0.5px solid ${GLASS_BORDER}`, display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            {hasOverrides && (
              <div style={{ padding: "6px 10px", borderRadius: 6, background: "rgba(234,179,8,0.08)", border: "0.5px solid rgba(234,179,8,0.3)", fontSize: 10, color: "#fbbf24" }}>
                Unsaved changes — {isUserDesign ? "save or discard below." : "save as custom design to keep them."}
              </div>
            )}
            ...
            <Button
              ...
              style={{ width: "100%", background: isUserDesign ? undefined : "rgba(99,102,241,0.15)", border: isUserDesign ? undefined : "0.5px solid rgba(99,102,241,0.35)" }}
            >
              Set as default for {activeDocType}s
            </Button>
            <button
              onClick={() => setShowSaveAs(v => !v)}
              style={{ width: "100%", padding: "7px 0", borderRadius: 8, fontSize: 12, fontWeight: 500, color: hasOverrides && !isUserDesign ? "#818cf8" : T2, background: hasOverrides && !isUserDesign ? "rgba(99,102,241,0.12)" : GLASS, border: `0.5px solid ${hasOverrides && !isUserDesign ? "rgba(99,102,241,0.4)" : GLASS_BORDER}`, cursor: "pointer" }}
            >
              {hasOverrides && !isUserDesign ? "Save as custom design ✱" : "Save as custom design"}
            </button>
            {showSaveAs && (
              <div style={{ display: "flex", gap: 6 }}>
                <input
            ...
            {hasOverrides && isUserDesign && (
              <button
                ...
                style={{ width: "100%", padding: "5px 0", borderRadius: 8, fontSize: 11, color: "#f87171", background: "rgba(248,113,113,0.06)", border: "0.5px solid rgba(248,113,113,0.2)", cursor: "pointer" }}
              >
                Discard changes
              </button>
            )}
        ...
        {/* ── Right: A4 preview ── */}
        <div style={{ flex: 1, background: "#111827", overflowY: "auto", display: "flex", flexDirection: "column", alignItems: "center", padding: "20px 20px 28px" }}>
          {/* Preview toolbar */}
          <div style={{ width: "min(595px, 100%)", marginBottom: 10, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1, minWidth: 0 }}>
              <span style={{ fontSize: 11, color: T3, textTransform: "uppercase", letterSpacing: "0.07em", whiteSpace: "nowrap" }}>Preview — {previewDesign.name}</span>
              {defaultDesignId === selectedDesignId && (
                <span style={{ fontSize: 9.5, padding: "2px 8px", borderRadius: 100, background: "rgba(99,102,241,0.15)", color: "#818cf8", border: "0.5px solid rgba(99,102,241,0.3)", whiteSpace: "nowrap" }}>
                  Default
                </span>
              )}
              {hasOverrides && (
                <span style={{ fontSize: 9.5, padding: "2px 8px", borderRadius: 100, background: "rgba(234,179,8,0.12)", color: "#fbbf24", border: "0.5px solid rgba(234,179,8,0.3)", whiteSpace: "nowrap" }}>
                  Unsaved
                </span>
              )}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 6, fontSize: 10, cursor: "pointer", border: `0.5px solid ${showMultiPage ? "rgba(99,102,241,0.5)" : GLASS_BORDER}`, background: showMultiPage ? "rgba(99,102,241,0.15)" : GLASS, color: showMultiPage ? "#818cf8" : T2 }}
              >
                <svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <rect x="1" y="1" width="4.5" height="6" rx="0.5" /><rect x="6.5" y="1" width="4.5" height="6" rx="0.5" /><line x1="1" y1="9" x2="5.5" y2="9" /><line x1="6.5" y1="9" x2="11" y2="9" />
                </svg>
                Pages
              </button>
              {/* Mock rows control (not for receipts) */}
              {activeDocType !== "receipt" && (
                <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "3px 8px", borderRadius: 6, border: `0.5px solid ${GLASS_BORDER}`, background: GLASS }}>
                  <svg width="10" height="10" viewBox="0 0 12 12" fill="none" stroke={T3} strokeWidth="1.5">
                    <line x1="1" y1="3" x2="11" y2="3" /><line x1="1" y1="6" x2="11" y2="6" /><line x1="1" y1="9" x2="11" y2="9" />
                  </svg>
                  <span style={{ fontSize: 10, color: T3, whiteSpace: "nowrap" }}>{mockRowCount} rows</span>
                  <input
          ...
          <div ref={previewWrapRef} style={{ boxShadow: "0 12px 60px rgba(0,0,0,0.7)", borderRadius: 4, overflowY: "scroll", width: "min(595px, 100%)" }}>
            <DocumentRenderer
