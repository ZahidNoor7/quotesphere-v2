...
export const PUT = withTenant("PUT /api/time-entries/[id]", async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  try {
...
export const DELETE = withTenant("DELETE /api/time-entries/[id]", async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  try {
    const session = await auth();
    if (!session) return NextResponse.json({ success: false, error: "Unauthorized" }, { status: 401 });
    const denied = requireRole(session, req.method);
