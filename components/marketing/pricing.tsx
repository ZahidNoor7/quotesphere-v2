...
export function Pricing({ plans }: { plans: PublicPlan[] }) {
  ...
  return (
    <Section id="pricing">
        ...
        {plans.map((plan, i) => {
          ...
          return (
            <StaggerItem key={plan.slug} className="h-full">
              <div
                className={cn(
                  "glass-card relative flex h-full flex-col gap-6 p-7 transition-transform duration-300 will-change-transform hover:-translate-y-1.5",
                  popular && "mkt-ring-glow",
                )}
                style={
                  popular
              ...
              >
                {popular && (
                  <span
                    className="absolute -top-3 left-1/2 inline-flex -translate-x-1/2 items-center gap-1 rounded-full px-3 py-1 text-[11px] font-semibold"
                    style={{
                      color: "var(--on-accent)",
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <p className="min-h-10 text-sm leading-relaxed" style={{ color: "var(--t2)" }}>
                    {plan.description}
                  </p>
                </div>

                <div className="flex items-baseline gap-1.5">
                  <span className="text-3xl font-bold tracking-tight tabular-nums" style={{ color: "var(--t1)" }}>
                    {main}
                  </span>
                  {suffix && (
                ...
                >
                  <Link href="/auth/register">{isFree ? "Get started" : "Start free trial"}</Link>
                </Button>
      ...
      <p className="mt-10 text-center text-sm" style={{ color: "var(--t2)" }}>
